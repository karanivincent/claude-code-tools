...
export async function applyRows(db, seedPlan, opts) {
  ...
  const flush = async () => {
    if (!batch) return;
    await db.upsert(batch.table, batch.rows);
    written += batch.rows.length;
    batch = null;
  };
  for (const r of ordered) {
    const values = { ...resolveValues(r.values, opts.now), id: r.id };
    if (batch && batch.table !== r.table) await flush();
    batch ??= { table: r.table, rows: [] };
