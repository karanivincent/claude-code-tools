...
| `tldr` | Replaces the end-of-task wall of prose with four fixed blocks — a two-line verdict, what the user must do, what went wrong, what shipped — under hard length ceilings, with the depth linked rather than inlined. Always prints the Needs-you block so its absence is never ambiguous, puts actionable commands in `bash` fences so the host renders a Run button, and avoids asterisk emphasis for clients that show the literal characters. Fires on merges, migrations, infra changes and multi-step investigations; stays silent for questions and trivial edits |
